...
class Crud_core {
	 ...
	 * @return mixed
	 */
	public function items($page_number, $per_page, $where, $order) {
		$offset     = $per_page * ( $page_number - 1 );
		return $this->model->getItems( $this->table, $where, $this->request, $this->schema, $this->fields, $order, $offset, $per_page );
	}
	...
	function view( $page_number, $per_page, $columns = null, $where = null, $order = null ) {
		...
		$items = $this->items( $page_number, $per_page, $where, $order);
	...
	function parent_form() {
		...
		if ( isset( $post['form'] ) ) {
			...
			if ( $this->validator->withRequest( $this->request )->run() || $novalidation ) //|| empty($this->fields)
			{
				...
				return [ 'redirect' => $this->base . '/' . $this->route . '/edit/' . $this->id ];
			} else {
				// echo '<div>';
		...
		if ( $this->multipart ) {
			$form .= form_open_multipart( '/' . $this->base . '/' . $this->route . '/' . ( $this->action == 'add' ? 'add' : 'edit/' . $this->id ) ) . '<div class="card-body">';
		} else {
			$form .= form_open( '/' . $this->base . '/' . $this->route . '/' . ( $this->action == 'add' ? 'add' : 'edit/' . $this->id ) ) . '<div class="card-body">';
		}
	...
	protected function input_wrapper( $field_type = null, $label, $input, $required ) {
		if ( ! $field_type ) {
			$output = '<label>' . $label . ':' . ( $required ? ' <span class="text-danger">*</span>' : '' ) . '</label>' . $input . '';
		} else {
	 ...
	 * @param $label
	 */
	protected function field_readonly( $label ) {
		$input = '';

		return $this->input_wrapper( null, $label, $input, false );
	}

	protected function field_hiden( $field_type, $label, $field_params = null ) {
		$input = '<input type="hidden" class="form-control" id="' . $field_type . '" name="' . $field_type . '"  value="' . set_value( $field_type, ( isset( $this->current_values->{$field_type} ) ? $this->current_values->{$field_type} : '' ) ) . '">';

		return $this->input_wrapper( $field_type, $label, $input, false );
	}
	...
	protected function field_text( $field_type, $label, $field_params ) {
		...
		$default = '';
		if ( isset( $field_params['default'] ) && $field_params['default'] ) {
			$default = $field_params['default'];
		}

		$input    = '<input type="text" ' . $required . ' class="form-control" id="' . $field_type . '" name="' . $field_type . '"  placeholder="" value="' . set_value( $field_type, ( isset( $this->current_values->{$field_type} ) ? $this->current_values->{$field_type} : $default ) ) . '" autocomplete="off">';
	...
	protected function field_select( $field_type, $label, $field_params, $values = null ) {
		$required = ( isset( $field_params['required'] ) && $field_params['required'] ? ' required ' : '' );
		$input    = '<select ' . $required . ' class="form-control" id="' . $field_type . '" name="' . $field_type . '">';

		if ( isset( $values ) && is_string( $values ) ) {
		...
		$input .= '</select><script>jQuery(document).ready(function() {
    jQuery("#' . $field_type . '").select2({allowClear: true,placeholder: "--",theme: "bootstrap4",width:"100%",minimumResultsForSearch:Infinity});
});</script>';
	...
	protected function field_dropdown( $field_type, $label, $field_params, $values ) {
		...
		$input       = '<select  class="form-control" ' . $required . ' id="' . $rid . '" name="' . $field_type . '">';

		if ( isset( $field_params['relation'] ) ) {
		...
		$input .= '</select><script>jQuery(document).ready(function() {
    jQuery("#' . $rid . '").select2({allowClear: true,placeholder: "--",theme: "bootstrap4",width:"100%",minimumResultsForSearch:Infinity});
});</script>';
    ...
    <script type="text/javascript">
            jQuery(function () {
                jQuery("#' . $field_type . '").datetimepicker({
	...
	protected function field_file( $field_type, $label, $field_params ) {
		...
		if ( isset( $field_params['wrapper_start'] ) && isset( $field_params['wrapper_end'] ) ) {
			...
			if ( $field_params['delete_callback'] ?? false ) {
				$deleteUrl    = $this->getBase() . '/' . $this->getRoute() . '/' . $field_params['delete_callback'] . '/' . $this->id;
				$deleteButton .= '<a 
                        onclick="return confirm(\'Bạn xác nhận xóa file này?\')" 
		...
		if ( isset( $field_params['wrapper_start'] ) && isset( $field_params['wrapper_end'] ) ) {
			...
			if ( $files = $this->files[ $field_type ] ?? null ) {
				...
				foreach ( $files as $file ) {
					...
					if ( $field_params['delete_callback'] ?? false ) {
						$deleteUrl    = $this->getBase() . '/' . $this->getRoute() . '/' . $field_params['delete_callback'] . '/' . $this->id . '/' . $file->{$relationOptions['primary_key']};
						$htmlFileName .= '<a 
                        onclick="return confirm(\'Bạn xác nhận xóa file này?\')" 
                        class="' . ( $field_params['delete_button_class'] ?? null ) . '" 
                        href="' . $deleteUrl . '">Delete</a>';
	...
	protected function items_table( $columns = null, $items, $pagination ) {
		...
		$table = '<div class="row" id="' . $this->table . '_content">
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">' . $this->table_title . '</h3>
                <div class="card-tools">
                  <a class="btn btn-primary btn-sm" href="' . $this->base . '/' . $this->route . '/add">' . $this->form_title_add . '</a>
                </div>
              </div>
              <!-- /.card-header -->
              <div class="card-body table-responsive p-0">' . form_open( '', [ 'method' => 'post', 'id' => 'search-inline' ] ) . form_close() . '
		...
		if ( $columns ) {
			...
			foreach ( $columns as $column ) {
				if ( is_array( $column ) ) {
					$label    = $column['label'];
					$th_class = w_url_title( strip_tags_content( $label ) );
					//$th_class = mb_strtolower (str_replace(' ', '-', $label));
				} else {
					$label    = $fields[ $column ]['label'] ?? ucfirst( str_replace( '_', ' ', $column ) );
					$th_class = $column;
				}

				$table .= '<th class="th th-' . $th_class . '">' . $label . '</th>';
			}
		} else {

			foreach ( $this->schema as $item ) {
				$label = $fields[ $item->Field ]['label'] ?? ucfirst( str_replace( '_', ' ', $item->Field ) );
				$table .= '<th class="th th-' . $item->Field . '">' . $label . '</th>';
			}
		}

		$table .= '<th class="th-action"><span class="hide">Actions</span></th>';
		...
		if ( $columns ) {
			...
			foreach ( $columns as $column ) {
				...
				if ( is_array( $column ) ) {
					...
					if ( $newCol = $column['search'] ?? false ) {
						...
						$label      = $column['label'];
						$table      .= '<td><input form="search-inline" type="' . $field_type . '" name="' . $newCol . '" class="form-control pull-right" value="' . set_value( $newCol ) . '"></td>';
						//$table .= '<td><input type="' . $field_type . '" name="' . $newCol . '" class="form-control pull-right" value="' . set_value($newCol) . '" placeholder="' . strip_tags_content($label) . '"></td>';
					} else {
						$table .= '<td></td>';
						continue;
					}
					...
					$label = $fields[ $column ]['label'] ?? ucfirst( str_replace( '_', ' ', $column ) );
					$table .= '<td><input form="search-inline" type="' . $field_type . '" name="' . $column . '" class="form-control pull-right" value="' . set_value( $column ) . '"></td>';
			...
			foreach ( $this->schema as $item ) {
				...
				$label = $fields[ $item->Field ]['label'] ?? ucfirst( str_replace( '_', ' ', $item->Field ) );
				$table .= '<td><input form="search-inline" type="' . $field_type . '" name="' . $item->Field . '" class="form-control pull-right" value="' . set_value( $item->Field ) . '"></td>';
				//$table .= '<td><input form="search-inline" type="' . $field_type . '" name="' . $item->Field . '" class="form-control pull-right" value="' . set_value($item->Field) . '" placeholder="' . $label . '"></td>';
			}
		}

		$table .= '<input form="search-inline" type="hidden" name="table_search" class="form-control pull-right" value="' . $this->table . '">';
		$table .= '<td class="text-center"><input form="search-inline" class="btn  btn-default" type="submit" value="Tìm kiếm"></td></tr>';
		...
		foreach ( $items as $item ) {
			...
			if ( $columns ) {
				foreach ( $columns as $column ) {
					...
					if ( is_array( $column ) ) {
						if ( $col_field = $column['field'] ?? false ) {
							$display_val = $item->{$col_field};
						} else {
					...
					$td_class = w_url_title( strip_tags_content( $label ) );
					$table .= '<td class="td td-' . $td_class . '">' . $display_val . '</td>';
				}
			} else {
			...
			$table .= '<td class="text-center"><a href="' . $this->base . '/' . $this->route . '/edit/' . $item->{$primary_key} . '" class="btn btn-success btn-sm">Sửa</a><a onclick="return confirm(\'Bạn xác nhận xóa nội dung này?\')" href="' . $this->base . '/' . $this->route . '/delete/' . $item->{$primary_key} . '" class="btn btn-danger btn-sm" style="margin-left: 10px;">Xóa</a></td>';
			$table .= '</tr>';
		}
		$table .= '</tbody></table></div>';
		$table .= '<div class="card-footer clearfix">';
		if ( $this->request->getPost( 'table_search' ) ) {
			$table .= '<a href="' . $this->base . '/' . $this->route . '" class="btn btn-warning btn-xs"><i class="fa fa-times"></i> Xóa bộ lọc</a>';
		} else {
			$table .= $pagination;
		}
	 ...
	 * @return bool
	 */
	public function checkItem( $table, $where ) {
		return $this->model->checkItem($table, $where);
	}
	...
	public function getRows($table, $where) {
		return $this->model->getRows($table, $where);
	}
